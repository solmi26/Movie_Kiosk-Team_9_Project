...
import movie.movieinfo.TheRoundup;
import movie.movieinfo.Avatar;
import movie.movieinfo.Whiplash;
import movie.movieinfo.TheDarkKnight;
import movie.movieinfo.SetMeFree;
         ...
         for(int j=0; j<timeTables.size();j++) {
            ...
            if(movies.get(i).getM_code() == timeTables.get(j).getM_code() && userDate.equals(date)) {               
               ...
               if(movies.get(i).getM_code()==1) {
                  timeButtons.get(count).addActionListener(new SetMeFree(timeButtons.get(count)));
               }else if(movies.get(i).getM_code()==2) {
                  timeButtons.get(count).addActionListener(new Avatar(timeButtons.get(count)));
               }else if(movies.get(i).getM_code()==3) {
                  timeButtons.get(count).addActionListener(new TheRoundup(timeButtons.get(count)));
               }else if(movies.get(i).getM_code()==4) {
                  timeButtons.get(count).addActionListener(new TheDarkKnight(timeButtons.get(count)));
               }else{
                  timeButtons.get(count).addActionListener(new Whiplash(timeButtons.get(count)));
               }
